  ...
  ngOnInit(): void {
    timer(0, 1000).pipe(
      ...
      if (diffMili.milliseconds <= 0) {
        this.renderer.setProperty(this.elementRef.nativeElement, 'innerHTML', '00:00:00');
        this.isFinished.next(true);
        this._destroyer$.next(true);
      } else {
        this.renderer.setProperty(this.elementRef.nativeElement, 'innerHTML', diffMili.toFormat('hh:mm:ss'));
      }
    });
  }
